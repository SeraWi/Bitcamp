...
public class FeedViewService {
	...
	// 피드 수정
	public int editFeed(int boardIdx, FeedEdit feedEdit, HttpServletRequest request) {

		int result = 0;

		dao = template.getMapper(FeedDao.class);

		result = dao.editFeed(feedEdit.getBoardDiscription(), feedEdit.getHashtag(), feedEdit.getTag(), boardIdx);

		return result;

	}

	// 2) 좋아요 상태인지 아닌지 확인 (세라 추가)
