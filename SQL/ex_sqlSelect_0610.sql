...
select ename, sal, sal+300 as newSal from emp;

--2. 사원의 이름, 급여, 연간 총 수입을 총 수입이 많은 것부터 작은 순으로 출력하시오,
--연간 총수입은 월급에 12를 곱한 후 $100의 상여금을 더해서 계산하시오.

select ename, sal, sal*12 +100 as ysal from emp order by ysal desc;
...
and ( deptno = 20 or deptno = 30) order by ename asc; 
...
select ename, job, sal from emp where ( job = 'CLERK' or job = 'SALESMAN') and
